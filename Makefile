...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"head_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f head-*.tar.gz
	echo Create archive \"head-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../head-$(VERSION)-$(BRANCH).tar.gz head
...
head.tar.gz: build
	cd make_build; tar czf ../head.tar.gz head && rm -rf ../make_build
